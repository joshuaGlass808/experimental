...
char* as_f_compound(AST_J* ast, list_J* list) {
  ...
  {
    AST_J* child_ast = (AST_J*) ast->children->items[i];
...
char* as_f_function(AST_J* ast, list_J* list)
{
  char* name = ast->name;
  ...
  {
    AST_J* farg = (AST_J*) as_val->children->items[i];
    AST_J* arg_variable = init_ast(AST_VARIABLE);
...
char* as_f_assignment(AST_J* ast, list_J* list)
{
...
char* as_f_variable(AST_J* ast, list_J* list) {
  char* s = calloc(1, sizeof(char));
...
char* as_f_call(AST_J* ast, list_J* list)
{
  char* s = calloc(1, sizeof(char));
  ...
  {
    AST_J* arg = (AST_J*)ast->value->children->items[i];
  ...
  if (list->size)
  {
    if (((AST_J*)list->items[0])->type == AST_ASSIGNMENT)
      addl_size = 0;
  }
...
char* as_f_statement_return(AST_J* ast, list_J* list)
{
  char* s = calloc(1, sizeof(char));
...
char* as_f_int(AST_J* ast, list_J* list)
{
...
char* as_f_string(AST_J* ast, list_J* list)
{
  list_J* chunks = str_to_hex_chunks(ast->string_value);
...
char* as_f_root(AST_J* ast, list_J* list)
{
...
char* as_f_access(AST_J* ast, list_J* list)
{
...
char* as_f_binop(AST_J* ast, list_J* list)
{
  char* s = calloc(1, sizeof(char));
...
char* as_f(AST_J* ast, list_J* list)
{
