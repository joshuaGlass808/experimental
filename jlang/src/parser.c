...
{
  parser_J* parser = calloc(1, sizeof(struct PARSER_STRUCT));
  parser->lexer = lexer;
...
AST_J* parser_parse_id(parser_J* parser)
{
  char* value = calloc(strlen(parser->token->value) + 1, sizeof(char));
  strcpy(value, parser->token->value);
  parser_eat(parser, TOKEN_ID); 

  AST_J* ast = init_ast(AST_VARIABLE);
  ast->name = value;
...
AST_J* parser_parse_block(parser_J* parser)
{
  parser_eat(parser, TOKEN_LBRACE);
  
  AST_J* ast = init_ast(AST_COMPOUND);
...
AST_J* parser_parse_list(parser_J* parser)
{
  ...
  if (parser->token->type == TOKEN_ARROW_RIGHT)
  {
    ...
    for (unsigned int i = 0; i < ast->children->size; i++)
      ((AST_J*)ast->children->items[i])->type = AST_ASSIGNMENT;
  }

  return ast;
}

AST_J* parser_parse_int(parser_J* parser)
{
  int int_value = atoi(parser->token->value);
  parser_eat(parser, TOKEN_INT);

  AST_J* ast = init_ast(AST_INT);
...
{
  AST_J* ast_left = parser_parse_factor(parser);
  ...
  {
    AST_J* ast_binop = init_ast(AST_BINOP);
...
{
  AST_J* ast_left = parser_parse_term(parser);
